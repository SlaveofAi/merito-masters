...
const Index = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('craftsman_profiles').select('*');
      if (error) {
  ...
  const filteredCraftsmen = craftsmen?.filter(craftsman => {
    const matchesSearch = craftsman.name.toLowerCase().includes(searchTerm.toLowerCase()) || craftsman.description?.toLowerCase().includes(searchTerm.toLowerCase()) || craftsman.trade_category.toLowerCase().includes(searchTerm.toLowerCase()) || craftsman.custom_specialization?.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesLocation = !locationFilter || craftsman.location.toLowerCase().includes(locationFilter.toLowerCase());
    const matchesCategory = categoryFilter === "Všetky kategórie" || craftsman.trade_category === categoryFilter || craftsman.custom_specialization === categoryFilter;
  ...
  return <Layout>
      ...
      <section className="py-16 bg-gray-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          {platformBenefits.map((benefit, index) => <div key={index} className="text-center group">
      ...
      {showSearchSection && <div ref={resultsRef} className="max-w-7xl mx-auto px-4 sm:px-6 py-16 bg-gray-50">
          <div className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
              <div className="md:col-span-5 relative group">
                <Search className={`absolute left-3 top-3 h-5 w-5 transition-all duration-300 z-10 ${isSearchFocused ? 'text-primary scale-110' : 'text-muted-foreground'}`} />
                <Input placeholder="Hľadajte podľa mena alebo remesla..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} onFocus={() => setIsSearchFocused(true)} onBlur={() => setIsSearchFocused(false)} className={`pl-10 transition-all duration-300 hover:shadow-md focus:shadow-lg focus:scale-[1.02] ${isSearchFocused ? 'border-primary/30' : ''}`} />
              </div>
              
              <div className="md:col-span-3 relative group">
                <MapPin className={`absolute left-3 top-3 h-5 w-5 transition-all duration-300 z-10 ${isLocationFocused ? 'text-primary scale-110' : 'text-muted-foreground'}`} />
                <Input placeholder="Zadajte lokalitu..." value={locationFilter} onChange={e => setLocationFilter(e.target.value)} onFocus={() => setIsLocationFocused(true)} onBlur={() => setIsLocationFocused(false)} className={`pl-10 transition-all duration-300 hover:shadow-md focus:shadow-lg focus:scale-[1.02] ${isLocationFocused ? 'border-primary/30' : ''}`} />
              </div>
              ...
              <div className="md:col-span-3">
                <Select value={categoryFilter} onValueChange={setCategoryFilter}>
                  ...
                  <SelectContent>
                    {getAllCategories().map(category => <SelectItem key={category} value={category} className="hover:bg-primary/5 transition-colors duration-200">
                        {category}
                      </SelectItem>)}
                  </SelectContent>
                </Select>
              ...
              <Button className="md:col-span-1 group relative overflow-hidden hover:shadow-lg active:scale-[0.98] transition-all duration-300" onClick={() => {
            setSearchTerm("");
            setLocationFilter("");
            ...
            <div className="mt-4 text-right">
              <Button variant="link" onClick={() => navigate('/categories')} className="text-primary hover:text-primary/80 hover:translate-x-1 transition-all duration-300">
          ...
          {isLoading ? <div className="flex justify-center items-center py-20">
            ...
            </div> : error ? <Card className="hover:shadow-md transition-shadow duration-300">
              <CardContent className="flex flex-col items-center justify-center py-10">
                <p className="text-center text-muted-foreground mb-4">
                  Nastala chyba pri načítaní remeselníkov. Skúste to prosím neskôr.
                </p>
                <Button onClick={() => window.location.reload()} className="hover:shadow-md active:scale-[0.98] transition-all duration-300">
                  Skúsiť znova
                </Button>
              </CardContent>
            </Card> : sortedCraftsmen?.length === 0 ? <Card className="hover:shadow-md transition-shadow duration-300">
              <CardContent className="flex flex-col items-center justify-center py-10">
                <p className="text-center text-muted-foreground mb-4">
            ...
            return <div key={craftsman.id} className="animate-fade-in hover:scale-[1.02] transition-all duration-300" style={{
              animationDelay: `${index * 100}ms`
            }}>
                      <CraftsmanCard id={craftsman.id} name={craftsman.name} profession={craftsman.custom_specialization || craftsman.trade_category} location={craftsman.location} imageUrl={craftsman.profile_image_url || getPlaceholderImage(craftsman.trade_category)} customSpecialization={craftsman.custom_specialization} isTopped={isTopped} />
                    </div>;
          })}
              </div>
            </>}
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-end mb-10">
            <div>
              <h2 className="text-3xl font-semibold tracking-tight mb-2">Objavte remeselníkov</h2>
          ...
          {craftsmen && craftsmen.length > 6 && <div className="flex justify-center mt-8">
              <Button onClick={() => {
            setShowSearchSection(true);
            setTimeout(() => {
              resultsRef.current?.scrollIntoView({
